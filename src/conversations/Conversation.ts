...
export class ConversationV1 {
  ...
  async decryptBatch(
    ...
    const decryptResponse = await this.client.keystore.decryptV1(
      this.buildDecryptRequest(messages)
    )
    ...
    console.log(
      'private key bundle: ',
      Buffer.from(this.client.keys.encode()).toString('base64')
    )
    ...
    console.log(
      'decrypt request: ',
      Buffer.from(decryptRequestBytes).toString('base64')
    )
    console.log(
      'expected base64 decryptResponse: ',
      Buffer.from(
        keystore.DecryptResponse.encode(decryptResponse).finish()
      ).toString('base64')
    )
    ...
    const fake_response_bytes = this.client.xmtplib?.decryptV1(
      this.client.xmtpHandle,
      decryptRequestBytes
    )
    ...
    for (let i = 0; i < fake_response.responses.length; i++) {
      ...
      console.log(
        'fake_response.responses[i].error',
        fake_response.responses[i].error
      )
    }
    // ==== DONE ===
