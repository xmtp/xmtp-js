...
function showHelp() {
  const customParams = {
    ...
    address: {
      ...
      type: "string" as const,
      description:
        "Ethereum address to find conversation for (find operation only, will be resolved to inbox ID)",
      required: false,
    },
  };
...
async function runFindOperation(config: Config): Promise<void> {
  if (!config.inboxId && !config.address) {
    ...
    console.log(`   Usage: yarn list find --inbox-id <inbox-id>`);
    console.log(`   Or: yarn list find --address <ethereum-address>`);
    return;
  }
  ...
  if (config.inboxId) {
    ...
    try {
      ...
      console.log(
        `📍 Resolved address ${config.address} to inbox ID: ${targetInboxId}`,
      );
    } catch (error) {
      const errorMessage =
  ...
  try {
    ...
    console.log(
      `   Searching ${conversations.length} conversations for ${identifierType}: ${config.inboxId || config.address}`,
    );
    console.log(`   Environment: ${process.env.XMTP_ENV ?? "production"}`);
    ...
    if (!foundConversation) {
      ...
      console.log(
        `   This ${identifierType} may not have any conversations with you yet.`,
      );
    ...
    console.log(
      `   Type: ${"groupName" in foundConversation ? "Group" : "Direct Message"}`,
    );
    console.log(
      `   URL: https://xmtp.chat/conversations/${foundConversation.id}`,
    );
