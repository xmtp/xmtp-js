...
describe("Group permissions", () => {
  ...
  it("should enforce add member policy", async () => {
    ...
    expect((await group2.members()).length).toBe(3);
    ...
    expect((await group2.members()).length).toBe(4);
    ...
    expect((await group2.members()).length).toBe(5);
    ...
    expect((await group2.members()).length).toBe(6);
  });
  ...
  it("should enforce remove member policy", async () => {
    ...
    expect((await group2.members()).length).toBe(2);
  });
  ...
  it("should enforce add admin policy", async () => {
    ...
    // verify default permissions - only super admins can add admins
    const permissions = await group.permissions();
    expect(permissions.policySet.addAdminPolicy).toBe(
      PermissionPolicy.SuperAdmin,
    );
    ...
    expect(await group2.isAdmin(client3.inboxId!)).toBe(true);
  });
  ...
  it("should enforce remove admin policy", async () => {
    ...
    // verify default permissions - only super admins can remove admins
    const permissions = await group.permissions();
    expect(permissions.policySet.removeAdminPolicy).toBe(
      PermissionPolicy.SuperAdmin,
    );
    ...
    expect(await group2.isAdmin(client3.inboxId!)).toBe(false);
  });
  ...
  it("should enforce update message disappearing policy with allow policy", async () => {
    ...
    // verify permissions
    const permissions = await group.permissions();
    expect(permissions.policySet.updateMessageDisappearingPolicy).toBe(
      PermissionPolicy.Allow,
    );
