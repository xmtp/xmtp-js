...
describe("Content types", () => {
  it("should send and receive text content", async () => {
    ...
    expect(textMessage.contentType).toEqual(await contentTypeText());
    ...
    expect(message?.contentType).toEqual(await contentTypeText());
  });
  ...
  it("should send and receive markdown content", async () => {
    ...
    expect(markdownMessage.contentType).toEqual(await contentTypeMarkdown());
    ...
    expect(message).toBeDefined();
    expect(message?.content).toBe("# gm");
    expect(message?.contentType).toEqual(await contentTypeMarkdown());
  });
  ...
  describe("Reaction", () => {
    it("should send and receive reaction content with added action", async () => {
      ...
      expect(decodedReaction.contentType).toEqual(await contentTypeReaction());
      ...
      const message = await client1.conversations.getMessageById(reactionId);
      expect(message).toBeDefined();
      expect(message?.content).toEqual(reaction);
      expect(message?.contentType).toEqual(await contentTypeReaction());
    });
  ...
  describe("Reply", () => {
    it("should send and receive reply with text content", async () => {
      ...
      const reply: XmtpReply = {
        content: await encodeText("This is a text reply"),
        reference: textMessageId,
        referenceInboxId: client1.inboxId!,
      };
      ...
      expect(await replyContent.contentType()).toEqual(await contentTypeText());
    ...
    it("should send and receive reply with non-text content (attachment)", async () => {
      ...
      const reply: XmtpReply = {
        reference: textMessageId,
        referenceInboxId: client1.inboxId!,
        content: await encodeAttachment(attachment),
      };
      ...
      expect(await replyContent.contentType()).toEqual(
        await contentTypeAttachment(),
      );
      expect(replyContent.inReplyTo).toBeDefined();
      expect(replyContent.inReplyTo?.id).toBe(textMessageId);
      expect(replyContent.inReplyTo?.content).toBe("Original message");
    ...
    it("should send and receive reply with custom content", async () => {
      ...
      expect(await replyContent.contentType()).toEqual(testCodec.contentType);
      expect(replyContent.inReplyTo).toBeDefined();
      expect(replyContent.inReplyTo?.id).toBe(textMessageId);
      expect(replyContent.inReplyTo?.content).toBe("Original message");
      expect(replyMessage.fallback).toBe(`Replied to an earlier message`);
  ...
  describe("Attachment", () => {
    it("should send and receive attachment content", async () => {
      ...
      expect(attachmentMessage.contentType).toEqual(
        await contentTypeAttachment(),
      );
      ...
      const message = await client1.conversations.getMessageById(attachmentId);
      expect(message).toBeDefined();
      expect(message?.content).toEqual(attachment);
      expect(message?.contentType).toEqual(await contentTypeAttachment());
    });
  ...
  describe("RemoteAttachment", () => {
    it("should encrypt and decrypt attachment content", async () => {
      const attachment: Attachment = {
        filename: "test.txt",
        mimeType: "text/plain",
        content: new Uint8Array([1, 2, 3]),
    ...
    it("should send and receive remote attachment content", async () => {
      ...
      const message =
        await client1.conversations.getMessageById(remoteAttachmentId);
      expect(message).toBeDefined();
      expect(message?.content).toEqual(remoteAttachment);
      expect(message?.contentType).toEqual(await contentTypeRemoteAttachment());
    });
  ...
  it("should send read receipts and get last read times", async () => {
    ...
    expect([...readTimes.keys()]).toContain(client1.inboxId!);
    expect(readTimes.get(client1.inboxId!)).toBeTypeOf("bigint");
    const message = await client1.conversations.getMessageById(readReceiptId);
    expect(message).toBeDefined();
    expect(message?.contentType).toEqual(await contentTypeReadReceipt());
    expect(message?.content).toEqual({});
    expect(isReadReceipt(message!)).toBe(true);
    if (isReadReceipt(message!)) {
      expectTypeOf(message).toEqualTypeOf<DecodedMessage<ReadReceipt>>();
    }
    ...
    expect([...readTimes2.keys()]).toContain(client1.inboxId);
    expect([...readTimes2.keys()]).toContain(client2.inboxId);
  });
  ...
  describe("TransactionReference", () => {
    it("should send and receive transaction reference content", async () => {
      ...
      expect(transactionReferenceMessage.contentType).toEqual(
        await contentTypeTransactionReference(),
      );
      ...
      const message = await client1.conversations.getMessageById(
  ...
  describe("WalletSendCalls", () => {
    it("should send and receive wallet send calls content", async () => {
      ...
      expect(walletSendCallsMessage.contentType).toEqual(
        await contentTypeWalletSendCalls(),
      );
      ...
      const message =
        await client1.conversations.getMessageById(walletSendCallsId);
      expect(message).toBeDefined();
      expect(message?.content).toEqual(walletSendCalls);
      expect(message?.contentType).toEqual(await contentTypeWalletSendCalls());
    });
  ...
  describe("Actions", () => {
    it("should send and receive actions", async () => {
      ...
      expect(actionsMessage.contentType).toEqual(await contentTypeActions());
      ...
      const message = await client1.conversations.getMessageById(actionsId);
      expect(message).toBeDefined();
      expect(message?.content).toEqual(actions);
      expect(message?.contentType).toEqual(await contentTypeActions());
    });
  ...
  describe("Intent", () => {
    it("should send and receive intent", async () => {
      ...
      expect(intentMessage.contentType).toEqual(await contentTypeIntent());
      ...
      const message = await client1.conversations.getMessageById(intentId);
      expect(message).toBeDefined();
      expect(message?.content).toEqual(intent);
      expect(message?.contentType).toEqual(await contentTypeIntent());
    });
  ...
  it("should send and receive group updated content", async () => {
    ...
    for (const message of messages) {
      expect(message.contentType).toEqual(await contentTypeGroupUpdated());
      ...
      const groupUpdated = await client1.conversations.getMessageById(
        message.id,
      );
      expect(groupUpdated?.content).toEqual(message.content);
      expect(groupUpdated?.contentType).toEqual(
