...
describe('RustWasmKeystore', () => {
  ...
  let aliceRustKeystore: libxmtp.Keystore
  let bobRustKeystore: libxmtp.Keystore
  ...
  beforeEach(async () => {
    ...
    // We use the InMemoryKeystore to test the Rust keystore in parallel
    const wasm = await libxmtp.XMTPWasm.initialize()
    aliceRustKeystore = wasm.newKeystoreWithBundle(
      PrivateKeyBundleV2.fromLegacyBundle(aliceKeys).encode()
    )
    bobRustKeystore = wasm.newKeystoreWithBundle(
      PrivateKeyBundleV2.fromLegacyBundle(bobKeys).encode()
    )
  })
  ...
  describe('decryptV1', () => {
    it('can decrypt a valid message', async () => {
      ...
      const { responses } = await aliceRustKeystore.decryptV1({ requests })
  ...
  describe('saveInvites', () => {
    it('can save a batch of valid envelopes', async () => {
      ...
      const { responses } = await bobRustKeystore.saveInvites({
        requests: [envelope],
      })
      ...
      const conversations = await bobRustKeystore.getV2Conversations()
  ...
  describe('encryptV2/decryptV2', () => {
    it('encrypts using a saved envelope', async () => {
      ...
      const sealedBytes = sealed.toBytes()
      const envelope = buildProtoEnvelope(sealedBytes, 'foo', created)
      await aliceRustKeystore.saveInvites({ requests: [envelope] })
      ...
      const {
        responses: [encrypted],
      } = await aliceRustKeystore.encryptV2({
        requests: [
          {
    ...
    it('round trips using a created invite', async () => {
      ...
      const createdNs = dateToNs(new Date())
      // Create key keystore.CreateInviteRequest proto
      const recipientProto = publicKey.SignedPublicKeyBundle.decode(
        recipient.toBytes()
      )
      const createInviteRequest: keystore.CreateInviteRequest =
        keystore.CreateInviteRequest.fromPartial({
          recipient: recipientProto,
          createdNs,
          context: undefined,
        })
      // Convert to v2 bundle
      const v2keyBundle = PrivateKeyBundleV2.fromLegacyBundle(aliceKeys)
      // encoded
      const encoded = v2keyBundle.encode()

      // Test createInvite on Rust but don't use it for now
      ...
      const {
        responses: [decrypted],
      } = await aliceRustKeystore.decryptV2({
        requests: [
          {
    ...
    it('correctly sorts conversations', async () => {
      ...
      const invites = Promise.all(
        shuffled.map((createdAt) => {
          return aliceRustKeystore.createInvite({
            recipient,
      ...
      const convos = await aliceRustKeystore.getV2Conversations()
